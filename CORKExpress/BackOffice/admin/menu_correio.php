...
session_start();
  ...
  if ($tipo == 1){
    $dado =mysqli_query($conn,"SELECT notificacoes.idnotifics, trabalhadores.nome, trabalhadores.apelido, notificacoes.texto, notificacoes.assunto, notificacoes.hours_notifics  FROM trabalhadores inner JOIN notificacoes ON trabalhadores.email = notificacoes.emaildestinatario WHERE notificacoes.emaildestinatario = '$dados[email]' AND notificacoes.emailremetente != '$dados[email]' ");
  }
  if ($tipo == 0){
